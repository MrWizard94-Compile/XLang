...
fn usage() {
    ...
    eprintln!(
        "ADR-129: aether.seed-bundle/v2 additionally accepts exactly one foundation-library -> bridge-library -> main-entry chain; it remains separate from general M11/M22 host elaboration."
    );
}
...
/// Invoke the closed ADR-128/129 bundle forge ABI on one caller-selected bundle file.
///
