...
honest seed limits), plus the post-0.37 SBP-001/SBP-002/SBP-003 bounded
seed-native two-unit, transitive-three-unit, and two-leaf-fan-in-four-unit
source profiles and post-0.36 M32a/M32b verified-execution evidence tooling.
M32a/M32b has no language or
...
ADR-128/129/130 use the distinct `compile_bundle [borrow bundle: Text] -> Bytes`
...
edges. v3 accepts exactly two ASCII/LF pure Whole leaves, one merge library, and
one entry under scalar framing, with three exact import edges: left and right
leaves feed merge, which feeds entry. The seed checks the frame, worlds, source
profile, path mangle, imports, and qualified calls before assembling one source
...
for the separately framed ADR-128/129/130 `aether.seed-bundle/v1`, `/v2`, and
`/v3` profiles, which use seed `compile_bundle` rather than the general module
path. See
...
Current bounded seed-bundle threat models: [SBP-001 v1](docs/Current%20state/THREAT_MODEL-SBP-001-SEED-BUNDLE.md), [SBP-002 v2](docs/Current%20state/THREAT_MODEL-SBP-002-SEED-CHAIN.md), and [SBP-003 v3](docs/Current%20state/THREAT_MODEL-SBP-003-SEED-FANIN.md).
