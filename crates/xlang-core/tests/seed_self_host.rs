...
fn barp_adr121_seed_speaks_canonical_root_nursery_zero_argument_spawn_unknown_calls() {
    ...
    let three_digit_argument = "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call nope 123 into result\n  yield result\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, three_digit_argument)
        .expect("three-digit root nursery spawn source must reach the seed forge");
    assert!(
        !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "three-digit root nursery spawn must remain outside the zero-argument pilot: {}",
        direct.stdout
    );
...
#[test]
fn barp_adr122_seed_speaks_canonical_root_nursery_single_digit_whole_spawn_unknown_calls() {
    let source = "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call nope 3 into result\n  yield result\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, source)
        .expect("canonical one-digit root nursery spawn unknown call must reach the seed forge");
    assert!(
        direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "expected a seed-native AE-SEED-011 packet for one-digit root nursery spawn, got: {}",
        direct.stdout
    );
    assert!(
        direct.stdout.contains("\"origin\":\"seed-speak\""),
        "expected seed-speak origin for one-digit root nursery spawn, got: {}",
        direct.stdout
    );
    assert!(
        direct
            .stdout
            .contains("\"schema\":\"aether.seed-error/v1\""),
        "expected seed error schema for one-digit root nursery spawn, got: {}",
        direct.stdout
    );
    assert!(
        direct.stdout.contains("\"line\":1,\"column\":1"),
        "expected 1:1 seed-SPEAK position for one-digit root nursery spawn, got: {}",
        direct.stdout
    );
    assert!(
        direct.stdout.contains(
        ...
        "expected the unknown-call message for one-digit root nursery spawn, got: {}",
        direct.stdout
    );
    assert_eq!(
        direct.stdout.matches("AETHER_SEED_ERROR:").count(),
        1,
        "canonical one-digit root nursery spawn source must emit exactly one packet: {}",
        direct.stdout
    );
    match direct.value {
        InvocationValue::Bytes(bytes) => assert!(
            bytes.is_empty(),
            "canonical one-digit root nursery spawn SPEAK must return blank Bytes, got {} bytes",
            bytes.len()
        ),
        other => {
            panic!("canonical one-digit root nursery spawn SPEAK must return Bytes, got {other:?}")
        }
    }

    let product = compile_product_bytecode(source).expect_err(
        "canonical one-digit root nursery spawn unknown call must fail product compilation",
    );
    assert!(
        product.to_string().contains("AE-SEED-011"),
        "expected AE-SEED-011 for one-digit root nursery spawn, got {product}"
    );
    let packets = product_error_packets(source);
    assert_eq!(packets.len(), 1);
    assert_eq!(packets[0].schema, SEED_ERROR_PACKET_SCHEMA);
    ...
    let valid = "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call worker 3 into result\n  yield result\n\ntask weave worker [limit: Whole] -> Whole:\n  checkpoint\n  yield limit\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, valid)
        .expect("declared one-digit root nursery spawn call must reach the seed forge");
    assert!(
        !direct.stdout.contains("AETHER_SEED_ERROR:"),
        "declared one-digit root nursery spawn call must remain outside the pilot: {}",
        direct.stdout
    );
    let seeded = bytes(direct);
    verify_bytecode(&seeded)
        .expect("declared one-digit root nursery spawn seed artifact must verify");
    let bootstrap = compile_to_bytecode(valid)
        .expect("declared one-digit root nursery spawn source must bootstrap")
        .bytecode;
    assert_eq!(
        seeded, bootstrap,
        "declared one-digit root nursery spawn call must retain seed/bootstrap identity"
    );
    assert_eq!(
        run_bytecode(&seeded)
            .expect("declared one-digit root nursery spawn seed artifact must run")
            .exit_code,
        3
    );
    ...
    for (boundary, spacer) in [
        ...
        let source = format!(
            "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n{spacer}    spawn call nope 3 into result\n  yield result\n"
        );
        let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, &source)
            .expect("delayed one-digit root nursery spawn source must reach the seed forge");
        assert!(
            !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
            "one-digit root nursery spawn after {boundary} must remain outside the immediate-child pilot: {}",
            direct.stdout
        );
    }
    ...
    let descendant = "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n      spawn call nope 3 into result\n  yield result\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, descendant)
        .expect("six-space one-digit nursery descendant source must reach the seed forge");
    assert!(
        !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "six-space one-digit nursery descendant must remain outside the four-space child pilot: {}",
        direct.stdout
    );

    let erroring_caller = "world w\n\nweave relay [] -> Whole raises Whole:\n  bind mutable result <- 0\n  together:\n    spawn call nope 3 into result\n  raise 0\n\nweave main [] -> Whole:\n  bind mutable value <- 0\n  bind mutable code <- 0\n  handle call relay into value otherwise error into code\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, erroring_caller)
        .expect("erroring one-digit root nursery spawn source must reach the seed forge");
    assert!(
        !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "erroring one-digit root nursery spawn must remain outside the total-caller pilot: {}",
        direct.stdout
    );

    let missing_world = "weave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call nope 3 into result\n  yield result\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, missing_world)
        .expect("missing-world one-digit root nursery spawn source must reach the seed forge");
    assert!(
        direct.stdout.contains("\"code\":\"AE-SEED-006\""),
        "missing world must retain higher-priority AE-SEED-006, got: {}",
        direct.stdout
    ...
    assert!(
        !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "missing world must suppress the lower-priority one-digit root nursery spawn pilot: {}",
        direct.stdout
    );
}

#[test]
fn barp_adr123_seed_speaks_canonical_root_nursery_two_digit_positive_whole_spawn_unknown_calls() {
    let source = "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call nope 42 into result\n  yield result\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, source)
        .expect("canonical two-digit root nursery spawn unknown call must reach the seed forge");
    assert!(
        direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "expected a seed-native AE-SEED-011 packet for two-digit root nursery spawn, got: {}",
        direct.stdout
    );
    assert!(
        direct.stdout.contains("\"origin\":\"seed-speak\""),
        "expected seed-speak origin for two-digit root nursery spawn, got: {}",
        direct.stdout
    );
    assert!(
        direct
            .stdout
            .contains("\"schema\":\"aether.seed-error/v1\""),
        "expected seed error schema for two-digit root nursery spawn, got: {}",
        direct.stdout
    );
    assert!(
        direct.stdout.contains("\"line\":1,\"column\":1"),
        "expected 1:1 seed-SPEAK position for two-digit root nursery spawn, got: {}",
        direct.stdout
    );
    assert!(
        direct.stdout.contains(
        ...
        "expected the unknown-call message for two-digit root nursery spawn, got: {}",
        direct.stdout
    );
    assert_eq!(
        direct.stdout.matches("AETHER_SEED_ERROR:").count(),
        1,
        "canonical two-digit root nursery spawn source must emit exactly one packet: {}",
        direct.stdout
    );
    match direct.value {
        InvocationValue::Bytes(bytes) => assert!(
            bytes.is_empty(),
            "canonical two-digit root nursery spawn SPEAK must return blank Bytes, got {} bytes",
            bytes.len()
        ),
        other => {
            panic!("canonical two-digit root nursery spawn SPEAK must return Bytes, got {other:?}")
        }
    }

    let product = compile_product_bytecode(source).expect_err(
        "canonical two-digit root nursery spawn unknown call must fail product compilation",
    );
    assert!(
        product.to_string().contains("AE-SEED-011"),
        "expected AE-SEED-011 for two-digit root nursery spawn, got {product}"
    );
    let packets = product_error_packets(source);
    assert_eq!(packets.len(), 1);
    assert_eq!(packets[0].schema, SEED_ERROR_PACKET_SCHEMA);
    ...
    for literal in ["10", "99"] {
        let boundary = format!(
            "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call nope {literal} into result\n  yield result\n"
        );
        let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, &boundary)
            .expect("two-digit lexical-boundary source must reach the seed forge");
        assert!(
            direct.stdout.contains("\"code\":\"AE-SEED-011\""),
            "positive two-digit literal {literal} must reach the seed witness: {}",
            direct.stdout
        );
    }

    let valid = "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call worker 42 into result\n  yield result\n\ntask weave worker [limit: Whole] -> Whole:\n  checkpoint\n  yield limit\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, valid)
        .expect("declared two-digit root nursery spawn call must reach the seed forge");
    assert!(
        !direct.stdout.contains("AETHER_SEED_ERROR:"),
        "declared two-digit root nursery spawn call must remain outside the pilot: {}",
        direct.stdout
    );
    let seeded = bytes(direct);
    verify_bytecode(&seeded)
        .expect("declared two-digit root nursery spawn seed artifact must verify");
    let bootstrap = compile_to_bytecode(valid)
        .expect("declared two-digit root nursery spawn source must bootstrap")
        .bytecode;
    assert_eq!(
        seeded, bootstrap,
        "declared two-digit root nursery spawn call must retain seed/bootstrap identity"
    );
    assert_eq!(
        run_bytecode(&seeded)
            .expect("declared two-digit root nursery spawn seed artifact must run")
            .exit_code,
        42
    );
    ...
    for noncanonical in ["00", "03"] {
        let source = format!(
            "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call nope {noncanonical} into result\n  yield result\n"
        );
        match forge_bytecode(SEED_COMPILER_ARTIFACT, &source) {
            Ok(direct) => assert!(
                !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
                "noncanonical two-digit literal {noncanonical} must not reach the target-existence pilot: {}",
                direct.stdout
            ),
            Err(error) => assert!(
                !error.to_string().contains("AE-SEED-011"),
                "noncanonical two-digit literal {noncanonical} must fail before the target-existence pilot: {error}"
            ),
        }
        let product = compile_product_bytecode(&source)
            .expect_err("a leading-zero two-digit literal must fail product compilation");
        assert!(
            !product.to_string().contains("AE-SEED-011"),
            "noncanonical two-digit literal {noncanonical} must retain literal priority over AE-SEED-011, got {product}"
        );
    }
    ...
    for (boundary, spacer) in [
        ...
        let source = format!(
            "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n{spacer}    spawn call nope 42 into result\n  yield result\n"
        );
        let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, &source)
            .expect("delayed two-digit root nursery spawn source must reach the seed forge");
        assert!(
            !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
            "two-digit root nursery spawn after {boundary} must remain outside the immediate-child pilot: {}",
            direct.stdout
        );
    }
    ...
    let descendant = "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n      spawn call nope 42 into result\n  yield result\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, descendant)
        .expect("six-space two-digit nursery descendant source must reach the seed forge");
    assert!(
        !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "six-space two-digit nursery descendant must remain outside the four-space child pilot: {}",
        direct.stdout
    );

    let non_task_target = "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call worker 42 into result\n  yield result\n\nweave worker [limit: Whole] -> Whole:\n  yield limit\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, non_task_target)
        .expect("non-task two-digit nursery target source must reach the seed forge");
    assert!(
        !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "declared non-task target must remain outside the target-existence pilot: {}",
        direct.stdout
    );

    let erroring_caller = "world w\n\nweave relay [] -> Whole raises Whole:\n  bind mutable result <- 0\n  together:\n    spawn call nope 42 into result\n  raise 0\n\nweave main [] -> Whole:\n  bind mutable value <- 0\n  bind mutable code <- 0\n  handle call relay into value otherwise error into code\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, erroring_caller)
        .expect("erroring two-digit root nursery spawn source must reach the seed forge");
    assert!(
        !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "erroring two-digit root nursery spawn must remain outside the total-caller pilot: {}",
        direct.stdout
    );

    let missing_world = "weave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call nope 42 into result\n  yield result\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, missing_world)
        .expect("missing-world two-digit root nursery spawn source must reach the seed forge");
    assert!(
        direct.stdout.contains("\"code\":\"AE-SEED-006\""),
        "missing world must retain higher-priority AE-SEED-006, got: {}",
        direct.stdout
    ...
    assert!(
        !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "missing world must suppress the lower-priority two-digit root nursery spawn pilot: {}",
        direct.stdout
    );
}

#[test]
fn barp_adr124_seed_speaks_canonical_root_nursery_bright_truth_spawn_unknown_calls() {
    let source = "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call nope bright into result\n  yield result\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, source)
        .expect("canonical bright root nursery spawn unknown call must reach the seed forge");
    assert!(
        direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "expected a seed-native AE-SEED-011 packet for bright root nursery spawn, got: {}",
        direct.stdout
    );
    assert!(
        direct.stdout.contains("\"origin\":\"seed-speak\""),
        "expected seed-speak origin for bright root nursery spawn, got: {}",
        direct.stdout
    );
    assert!(
        direct
            .stdout
            .contains("\"schema\":\"aether.seed-error/v1\""),
        "expected seed error schema for bright root nursery spawn, got: {}",
        direct.stdout
    );
    assert!(
        direct.stdout.contains("\"line\":1,\"column\":1"),
        "expected 1:1 seed-SPEAK position for bright root nursery spawn, got: {}",
        direct.stdout
    );
    assert!(
        direct.stdout.contains(
        ...
        "expected the unknown-call message for bright root nursery spawn, got: {}",
        direct.stdout
    );
    assert_eq!(
        direct.stdout.matches("AETHER_SEED_ERROR:").count(),
        1,
        "canonical bright root nursery spawn source must emit exactly one packet: {}",
        direct.stdout
    );
    match direct.value {
        InvocationValue::Bytes(bytes) => assert!(
            bytes.is_empty(),
            "canonical bright root nursery spawn SPEAK must return blank Bytes, got {} bytes",
            bytes.len()
        ),
        other => {
            panic!("canonical bright root nursery spawn SPEAK must return Bytes, got {other:?}")
        }
    }

    let product = compile_product_bytecode(source).expect_err(
        "canonical bright root nursery spawn unknown call must fail product compilation",
    );
    assert!(
        product.to_string().contains("AE-SEED-011"),
        "expected AE-SEED-011 for bright root nursery spawn, got {product}"
    );
    let packets = product_error_packets(source);
    assert_eq!(packets.len(), 1);
    assert_eq!(packets[0].schema, SEED_ERROR_PACKET_SCHEMA);
    ...
    let valid = "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call worker bright into result\n  yield result\n\ntask weave worker [flag: Truth] -> Whole:\n  checkpoint\n  yield 42\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, valid)
        .expect("declared bright root nursery spawn call must reach the seed forge");
    assert!(
        !direct.stdout.contains("AETHER_SEED_ERROR:"),
        "declared bright root nursery spawn call must remain outside the pilot: {}",
        direct.stdout
    );
    let seeded = bytes(direct);
    verify_bytecode(&seeded).expect("declared bright root nursery spawn seed artifact must verify");
    let bootstrap = compile_to_bytecode(valid)
        .expect("declared bright root nursery spawn source must bootstrap")
        .bytecode;
    assert_eq!(
        seeded, bootstrap,
        "declared bright root nursery spawn call must retain seed/bootstrap identity"
    );
    assert_eq!(
        run_bytecode(&seeded)
            .expect("declared bright root nursery spawn seed artifact must run")
            .exit_code,
        42
    );

    let assert_no_bright_pilot =
        |source: &str, boundary: &str| match forge_bytecode(SEED_COMPILER_ARTIFACT, source) {
            Ok(direct) => assert!(
                !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
                "{boundary} must remain outside the bright pilot: {}",
                direct.stdout
            ),
            Err(error) => assert!(
            !error.to_string().contains("AE-SEED-011"),
            "{boundary} must not emit the bright pilot before its existing seed failure: {error}"
        ),
        };
    ...
    for (boundary, spacer) in [
        ...
        let source = format!(
            "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n{spacer}    spawn call nope bright into result\n  yield result\n"
        );
        assert_no_bright_pilot(
            &source,
            &format!("bright root nursery spawn after {boundary}"),
        );
    }

    let descendant = "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n      spawn call nope bright into result\n  yield result\n";
    assert_no_bright_pilot(descendant, "six-space bright nursery descendant");

    let non_task_target = "world w\n\nweave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call worker bright into result\n  yield result\n\nweave worker [flag: Truth] -> Whole:\n  yield 42\n";
    assert_no_bright_pilot(non_task_target, "declared non-task target");
    ...
    let product = compile_product_bytecode(wrong_parameter)
        .expect_err("wrong-parameter bright nursery target must fail product compilation");
    assert!(
        !product.to_string().contains("AE-SEED-011"),
    ...
    let erroring_caller = "world w\n\nweave relay [] -> Whole raises Whole:\n  bind mutable result <- 0\n  together:\n    spawn call nope bright into result\n  raise 0\n\nweave main [] -> Whole:\n  bind mutable value <- 0\n  bind mutable code <- 0\n  handle call relay into value otherwise error into code\n";
    assert_no_bright_pilot(erroring_caller, "erroring bright root nursery spawn");

    let missing_world = "weave main [] -> Whole:\n  bind mutable result <- 0\n  together:\n    spawn call nope bright into result\n  yield result\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, missing_world)
        .expect("missing-world bright root nursery spawn source must reach the seed forge");
    assert!(
        direct.stdout.contains("\"code\":\"AE-SEED-006\""),
        "missing world must retain higher-priority AE-SEED-006, got: {}",
        direct.stdout
...
#[test]
fn barp_adr127_seed_speaks_canonical_root_comptime_bind_unknown_calls() {
    let source = "world w\n\nweave main [] -> Whole:\n  comptime bind result <- call nope 3\n  yield result\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, source)
        .expect("canonical comptime-bind unknown call must reach the seed forge");
    assert!(
        direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "expected a seed-native AE-SEED-011 packet for root comptime bind, got: {}",
        direct.stdout
    );
    assert!(
        direct.stdout.contains("\"origin\":\"seed-speak\""),
        "expected seed-speak origin for root comptime bind, got: {}",
        direct.stdout
    );
    assert!(
        direct
            .stdout
            .contains("\"schema\":\"aether.seed-error/v1\""),
        "expected seed error schema for root comptime bind, got: {}",
        direct.stdout
    );
    assert!(
        direct.stdout.contains("\"line\":1,\"column\":1"),
        "expected 1:1 seed-SPEAK position for root comptime bind, got: {}",
        direct.stdout
    );
    assert!(
        direct.stdout.contains(
        ...
        "expected the unknown-call message for root comptime bind, got: {}",
        direct.stdout
    );
    assert_eq!(
        direct.stdout.matches("AETHER_SEED_ERROR:").count(),
        1,
        "canonical root comptime bind source must emit exactly one packet: {}",
        direct.stdout
    );
    match direct.value {
        InvocationValue::Bytes(bytes) => assert!(
            bytes.is_empty(),
            "canonical root comptime bind SPEAK must return blank Bytes, got {} bytes",
            bytes.len()
        ),
        other => panic!("canonical root comptime bind SPEAK must return Bytes, got {other:?}"),
    }

    let product = compile_product_bytecode(source)
        .expect_err("canonical root comptime-bind unknown call must fail product compilation");
    assert!(
        product.to_string().contains("AE-SEED-011"),
        "expected AE-SEED-011 for root comptime bind, got {product}"
    );
    let packets = product_error_packets(source);
    assert_eq!(packets.len(), 1);
    assert_eq!(packets[0].schema, SEED_ERROR_PACKET_SCHEMA);
    ...
    let zero_argument =
        "world w\n\nweave main [] -> Whole:\n  comptime bind result <- call nope\n  yield result\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, zero_argument)
        .expect("zero-argument comptime-bind unknown call must reach the seed forge");
    assert!(
        direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "expected a seed-native AE-SEED-011 packet for zero-argument root comptime bind, got: {}",
        direct.stdout
    );
    assert!(
        direct.stdout.contains("\"origin\":\"seed-speak\""),
        "expected seed-speak origin for zero-argument root comptime bind, got: {}",
        direct.stdout
    );
    assert_eq!(
        direct.stdout.matches("AETHER_SEED_ERROR:").count(),
        1,
        "zero-argument root comptime bind source must emit exactly one packet: {}",
        direct.stdout
    );
    match direct.value {
        InvocationValue::Bytes(bytes) => assert!(
    ...
    let valid = "world w\n\nweave triple [value: Whole] -> Whole:\n  bind product_value <- product value 3\n  yield product_value\n\nweave main [] -> Whole:\n  comptime bind result <- call triple 14\n  yield result\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, valid)
        .expect("declared root comptime-bind call must reach the seed forge");
    assert!(
        !direct.stdout.contains("AETHER_SEED_ERROR:"),
        "declared root comptime-bind call must remain outside the pilot: {}",
        direct.stdout
    );
    let seeded = bytes(direct);
    verify_bytecode(&seeded)
    ...
    assert_eq!(
        seeded, bootstrap,
        "declared root comptime-bind call must retain seed/bootstrap identity"
    );
    assert_eq!(
        run_bytecode(&seeded)
            .expect("declared root comptime-bind seed artifact must run")
            .exit_code,
        42
    );

    let valid_zero_argument = "world w\n\nweave answer [] -> Whole:\n  yield 42\n\nweave main [] -> Whole:\n  comptime bind result <- call answer\n  yield result\n";
    let direct = forge_bytecode(SEED_COMPILER_ARTIFACT, valid_zero_argument)
        .expect("declared zero-argument root comptime-bind call must reach the seed forge");
    assert!(
        !direct.stdout.contains("AETHER_SEED_ERROR:"),
        "declared zero-argument root comptime-bind call must remain outside the pilot: {}",
        direct.stdout
    );
    let seeded = bytes(direct);
    verify_bytecode(&seeded)
        .expect("declared zero-argument root comptime-bind seed artifact must verify");
    let bootstrap = compile_to_bytecode(valid_zero_argument)
        .expect("declared zero-argument root comptime-bind source must bootstrap")
        .bytecode;
    assert_eq!(
        seeded, bootstrap,
        "declared zero-argument root comptime-bind call must retain seed/bootstrap identity"
    );
    assert_eq!(
        run_bytecode(&seeded)
            .expect("declared zero-argument root comptime-bind seed artifact must run")
            .exit_code,
        42
    );

    let assert_no_comptime_bind_pilot = |source: &str, boundary: &str| {
        match forge_bytecode(SEED_COMPILER_ARTIFACT, source) {
            Ok(direct) => assert!(
                !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
                "{boundary} must remain outside the comptime-bind pilot: {}",
                direct.stdout
            ),
            Err(error) => assert!(
                !error.to_string().contains("AE-SEED-011"),
                "{boundary} must not emit the comptime-bind pilot before its existing seed failure: {error}"
            ),
        }
    };
    ...
    assert!(
        !direct.stdout.contains("\"code\":\"AE-SEED-011\""),
        "missing world must suppress the lower-priority comptime-bind pilot: {}",
        direct.stdout
    );
}
...
fn barp_adr072_product_seed_error_packet_abi() {
    ...
    assert!(
        aether_core::seed_speak_emit_root_nursery_single_digit_whole_spawn_unknown_call_pilot(),
        "ADR-122: root nursery single-digit Whole spawn-call unknown-call seed SPEAK pilot"
    );
    assert!(
        aether_core::seed_speak_emit_root_nursery_two_digit_positive_whole_spawn_unknown_call_pilot(
        ),
        "ADR-123: root nursery positive two-digit Whole spawn-call unknown-call seed SPEAK pilot"
    );
    assert!(
