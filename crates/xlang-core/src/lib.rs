...
pub use native::{
    ...
    native_supported_target_triples, native_target_is_host, native_target_is_supported,
...
pub use registry::{
    ...
    verify_x509_lite_certificate, verify_x509_lite_store, verify_x509_lite_store_chain,
    write_trust_policy, write_x509_lite_store, RegistryCacheDocument, RegistryError,
    RegistryPackagePin, RegistryTrustDocument, RegistryTrustKey, RegistryTrustPolicy,
    RegistryX509LiteCert, RegistryX509LiteStore, RegistryX509LiteTbs, REGISTRY_ALG_ED25519,
    REGISTRY_ALG_HMAC_SHA256, REGISTRY_CACHE_SCHEMA, REGISTRY_INDEX_FILE, REGISTRY_TRUST_FILE,
    REGISTRY_TRUST_POLICY_FILE, REGISTRY_TRUST_POLICY_SCHEMA, REGISTRY_TRUST_SCHEMA,
    REGISTRY_X509_LITE_PEM_BEGIN, REGISTRY_X509_LITE_PEM_END, REGISTRY_X509_LITE_SCHEMA,
    REGISTRY_X509_LITE_STORE_FILE, REGISTRY_X509_LITE_STORE_SCHEMA,
};
pub use workspace::{
...
/// ADR-128/129/130 product route for bounded seed-native Whole-library source bundles.
///
...
pub fn compile_product_seed_bundle(bundle: &str) -> Result<Vec<u8>, CompilerError> {
    ...
    debug_assert!(
        seed_native_whole_library_fanin_bundle_profile(),
        "ADR-130: bounded seed-native fan-in bundle profile must remain enabled"
    );
    debug_assert!(
...
/// ADR-130: the seed natively elaborates the deliberately bounded SBP-003
/// four-unit two-leaf-to-merge-to-entry fan-in profile through the same closed
/// `compile_bundle` forge ABI.
///
/// This must never be used to overstate the general M11/M22 tracker.
#[must_use]
pub const fn seed_native_whole_library_fanin_bundle_profile() -> bool {
    true
}
...
/// ADR-098 / ADR-102 / ADR-103 / ADR-106 / ADR-108 / ADR-109 / ADR-110 / ADR-111 / ADR-112 / ADR-113 / ADR-114 / ADR-115 / ADR-116 / ADR-117 / ADR-118 / ADR-119 / ADR-120 / ADR-121 / ADR-122 / ADR-123 / ADR-124 / ADR-125 / ADR-126: seed.ae SPEAK pilot codes (subset of the
/// conformance matrix).
#[must_use]
...
/// ADR-110/111/116/117/118/119/120/121/122/123/124/125/126: the checked-in seed line-scans
...
/// whose immediate child is either a zero-argument `spawn call target into
/// destination`, a one-digit decimal-Whole `spawn call target 3 into
/// destination`, or a positive two-digit decimal-Whole `spawn call target 42
/// into destination`. It verifies only their target against canonical top-level
/// declaration headers. The direct-form witness accepts one-or-more Whole arguments
...
/// `bright` Truth literal, ADR-125 requires the exact `dim` Truth literal, and
/// ADR-126 requires the exact empty Text literal before that delimiter. It is
/// not name binding,
...
/// ADR-117: canonical root `revise name <- call target` forms reach the same
/// seed-side target-existence witness as direct bind and root-yield forms. This
/// is intentionally not a statement parser or a destination/type/effect check.
#[must_use]
pub const fn seed_speak_emit_revise_unknown_call_pilot() -> bool {
    true
}

/// ADR-118: canonical root `speak call target` forms reach the same seed-side
/// target-existence witness as direct bind, root yield, and root revise forms.
/// This is intentionally not an expression parser or a Text-result/type/effect
/// check.
#[must_use]
...
/// ADR-120: canonical root `forward call target` forms under the literal
/// erroring-Whole header marker reach the same seed-side target-existence
/// witness as the total-caller direct forms. This is intentionally not a full
...
/// ADR-122: a literal total-Whole root `together:` followed immediately by a
/// four-space `spawn call target digit into destination` line reaches the
/// seed-side target-existence witness only when `digit` is exactly one ASCII
/// decimal character. This intentionally does not validate general arguments,
/// task identity, destination legality, nesting, effect, or resource policy.
#[must_use]
pub const fn seed_speak_emit_root_nursery_single_digit_whole_spawn_unknown_call_pilot() -> bool {
    true
}

/// ADR-123: a literal total-Whole root `together:` followed immediately by a
/// four-space `spawn call target digits into destination` line reaches the
/// seed-side target-existence witness only when `digits` is exactly two ASCII
/// decimal characters and its first character is nonzero. This intentionally
/// does not validate general arguments, task identity, destination legality,
/// nesting, effect, or resource policy.
#[must_use]
pub const fn seed_speak_emit_root_nursery_two_digit_positive_whole_spawn_unknown_call_pilot() -> bool
{
    true
}

/// ADR-124: a literal total-Whole root `together:` followed immediately by a
/// four-space `spawn call target bright into destination` line reaches the
/// seed-side target-existence witness only for the exact `bright` Truth literal
...
/// ADR-126: a literal total-Whole root `together:` followed immediately by a
/// four-space `spawn call target "" into destination` line reaches the
/// seed-side target-existence witness only for the exact empty Text literal and
/// a nonempty destination suffix. This intentionally does not validate nonempty
/// or escaped Text, bytes, names, signature, destination legality, nesting,
/// effect, or resource policy.
#[must_use]
pub const fn seed_speak_emit_root_nursery_empty_text_spawn_unknown_call_pilot() -> bool {
    true
}
...
mod product_task_frame_surface_tests {
    ...
    fn seed_speak_pilot_covers_lexical_structural_and_task_preflights() {
        ...
        assert!(seed_speak_emit_root_nursery_empty_text_spawn_unknown_call_pilot());
