Below is a single-file, zero-dependency Rust front-end that includes:

- a lexer

- a recursive-descent AST parser

- an explicit type system

- compile-time type checking with simple inference for `let x = expr;`

It uses a deliberately small, brace-delimited syntax so the core compiler pieces stay clean and easy to extend.
...
Ok(Token { kind: token, span: start })
...
span: fn_tok.span.max(name_span),
...
self.expect(SimpleKind::Semicolon, "expected `;` after variable declaration")?;
...
self.expect(SimpleKind::Semicolon, "expected `;` after return expression")?;
...
self.expect(SimpleKind::RParen, "expected `)` after grouped expression")?;
...
let current = self.scopes.last_mut().expect("type environment always has one scope");
...
// Pass 2: sequentially check top-level statements and function bodies.
...
format!("function `{}` conflicts with an existing global variable", func.name),
...
format!("`{}` is a function name and cannot be used as a parameter", param.name),
...
If you want, I can next extend this into one of these directions:

1. **Indentation-sensitive Python-style blocks**

2. **A typed AST / HIR lowering pass**

3. **A borrow-check-style lifetime system**

4. **Bytecode generation or a direct interpreter**

5. **Structs, enums, pattern matching, and generics**
