...
are product-surface (ADR-066). **ADR-064–067:** default CLI check/format/structure/
project format, LSP format, product weave-replace structural edit, and seed rebuild
use the product seed path; bootstrap is recovery (`--bootstrap`), dual-compare
oracle, and residual non–weave-replace structural AST only.
