...
**Date:** 2026-08-04 (Aether 0.15 / M11 modules complete)
...
| CLM-021 | Fine-grained structural edits (statement-level paths under weave bodies via `aether.edit/v7`) are the accepted M12 authoring direction; expression-atom surgery and general JSONPath are out of scope. | **Accepted direction (designed; not implemented)** | [M12 design](DESIGN-M12-FINE-GRAINED-EDITS.md), [ADR-016](ADR-016-m12-fine-grained-edits.md), [M12 validation matrix](M12-VALIDATION-MATRIX.md). | Implement v7 apply-edit; matrix green; preserve seed-before-write. |
