...
**Date:** 2026-08-04 (Aether 0.18 / M14 host I/O designed)
...
| CLM-023 | Capability-mediated host I/O (grant-backed read/write/env host weaves; deny-by-default; no ambient FS/shell/network) is the accepted M14 direction under threat model v2. | **Accepted direction (designed; not implemented)** | [THREAT_MODEL-v2-CAPABLE-HOST.md](THREAT_MODEL-v2-CAPABLE-HOST.md), [M14 design](DESIGN-M14-HOST-IO-CAPABILITIES.md), [ADR-018](ADR-018-m14-host-io-capabilities.md), [M14 validation matrix](M14-VALIDATION-MATRIX.md). | Implement grant table + catalog; path-jail negatives; pure `run` unchanged without grants. |
