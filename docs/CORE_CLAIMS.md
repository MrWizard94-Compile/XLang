...
**Date:** 2026-08-04 (Aether 0.14 / M11a modules bootstrap)
...
| CLM-020 | Language modules (`import unit` / `export weave` / qualified calls, lib without main, `project build`) work for multi-module projects via **bootstrap** multi-source elaboration (M11a); single-file seed compile rejects imports. | **Proven now, bounded M11a (bootstrap-hosted multi-module)** | [AETHER_0.12-MODULES.md](AETHER_0.12-MODULES.md), [ADR-015](ADR-015-m11-language-modules.md), module tests, `examples/project-modules`. | M11b: seed≡bootstrap on module corpus before seed-hosted multi-module authority. |
