...
**Date:** 2026-08-04 (Aether 0.12 / language surface 0.11 / M9 update)
...
| CLM-017 | Offline `aether.project/v1` documents with optional SHA-256 locks and CLI format/project verify provide local project integrity without a network registry or LSP. | **Proven now, bounded M9 scope** | [AETHER_0.12.md](AETHER_0.12.md), [M9 design](DESIGN-M9-PROJECT-TOOLING.md), [ADR-012](ADR-012-m9-project-tooling.md), project module tests, `examples/project`. | M10 multi-unit nested integrity is designed ([ADR-013](ADR-013-m10-multi-unit-projects.md)); prove matrix before claiming multi-unit Done. |
| CLM-018 | Multi-unit offline projects (nested paths, locks, independent per-unit seed compile, no language modules) are the accepted M10 tooling direction. | **Accepted direction (designed; not implemented)** | [M10 design](DESIGN-M10-MULTI-UNIT-PROJECTS.md), [ADR-013](ADR-013-m10-multi-unit-projects.md), [M10 validation matrix](M10-VALIDATION-MATRIX.md). | Green matrix, multi-unit example, `project format`, DOC-SYNC; still no import/registry. |
