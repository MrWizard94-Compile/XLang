...
| CLM-020 | Language modules (`import unit` / `export weave` / qualified calls, lib without main, project build) are the accepted M11 direction, with bootstrap multi-module first (M11a) and seed dual-compare required before seed-hosted multi-module authority (M11b). | **Accepted direction (designed; not implemented)** | [M11 design](DESIGN-M11-LANGUAGE-MODULES.md), [ADR-015](ADR-015-m11-language-modules.md), [M11 validation matrix](M11-VALIDATION-MATRIX.md). | M11a matrix green + honest MANIFEST; M11b seed≡bootstrap before claiming seed multi-module. |
