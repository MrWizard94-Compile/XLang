...
**Date:** 2026-08-04 (Aether 0.16 / M12 fine-grained edits)
...
| CLM-022 | A bounded offline Language Server (`aether lsp`) is the accepted M13 direction: bootstrap diagnostics, symbols, format, hover, definition; not a second product compiler; no silent disk writes or network. | **Accepted direction (designed; not implemented)** | [M13 design](DESIGN-M13-BOUNDED-LSP.md), [ADR-017](ADR-017-m13-bounded-lsp.md), [M13 validation matrix](M13-VALIDATION-MATRIX.md). | Implement M13a matrix; keep seed as product compile authority. |
