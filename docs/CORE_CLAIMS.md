...
**Date:** 2026-08-01 (Aether 0.7 M4 update)
...
| CLM-002 | Default CLI compilation is seed-hosted; the Rust core is bootstrap/diagnostic authority; the documented canonical 0.7 surface, including M2 and M4 corpora, has seed/bootstrap byte-identity proof. | **Proven now** | [SEED_PROFILE.md](SEED_PROFILE.md), [seed self-host tests](../crates/xlang-core/tests/seed_self_host.rs), resource/effect corpus. | Extend the proof corpus before any new source form joins the product compile path. |
...
| CLM-005 | Aether 0.7 tooling provides a versioned semantic AST, code/span diagnostics, and validated local top-level structural edits in addition to canonical text. | **Proven now, bounded M3/M4 scope** | [AETHER_AUTHORING_PROTOCOL_v2.md](AETHER_AUTHORING_PROTOCOL_v2.md), [ADR-005](ADR-005-structural-authoring-contract.md), core/CLI contract tests. | Preserve corpus/negative proof for every new node or operation; separately design fine-grained edits. |
| CLM-006 | Aether 0.7 bounded resources make allocation explicit and capability-oriented rather than introducing an ambient language-level allocator. | **Proven now, bounded M2 scope** | [AETHER_0.7.md](AETHER_0.7.md), [ADR-004](ADR-004-aeth-v6-bounded-resources.md), resource behavior and hostile-artifact tests. | Specify and prove first-class outcome propagation before extending resource boundaries. |
| CLM-012 | Aether 0.7 carries typed resource facts into bytecode emission and verifies Buffer operand provenance before VM execution. | **Proven now, bounded M2 scope** | [AETHER_0.7.md](AETHER_0.7.md), [ADR-004](ADR-004-aeth-v6-bounded-resources.md), `SemanticResourcePlan` and verifier tests. | Preserve the same proof standard for any additional resource type or cross-weave result. |
| CLM-007 | Aether 0.7 implements a bounded, abortive `Error[Whole]` effect with visible handled, forwarded, and rejected routes and no hidden exception path. | **Proven now, bounded M4 scope** | [AETHER_0.7.md](AETHER_0.7.md), [M4 design](DESIGN-M4-TYPED-ERROR-EFFECTS.md), [ADR-007](ADR-007-m4-typed-error-effect.md), [M4 validation matrix](M4-VALIDATION-MATRIX.md), core/seed/authoring tests. | Preserve hostile artifact, seed-byte-identity, and total-entry proof; separately prove any resource/cancellation interaction before relaxing the clean boundary. |
