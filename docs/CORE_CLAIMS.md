...
**Date:** 2026-08-04 (Aether 0.17 / M13a bounded LSP)
...
| CLM-022 | A bounded offline Language Server (`aether lsp`) provides bootstrap diagnostics, symbols, format, hover, and definition over stdio; it is not a second product compiler and does not write disk or emit AETH. | **Proven now, bounded M13a** | [DESIGN-M13-BOUNDED-LSP.md](DESIGN-M13-BOUNDED-LSP.md), [ADR-017](ADR-017-m13-bounded-lsp.md), CLI `lsp` module tests. | M13b cross-file imports optional; keep seed as product compile authority. |
