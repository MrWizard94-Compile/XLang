...
| Current seed proof covers documented canonical 0.5 source only. | [SEED_PROFILE.md](../Current%20state/SEED_PROFILE.md) | Every accepted M2 source form needs its own bootstrap/seed byte-identity corpus. |
| VM/forge accept verified AETH only and expose no host capability to guest code. | [ARCHITECTURE.md](../Current%20state/ARCHITECTURE.md), [MANIFEST.md](../../MANIFEST.md) | Arena instructions must remain VM-private and be verifier-gated before any run/write. |
