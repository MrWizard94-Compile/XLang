...
unknown target has stable target-existence priority, while `dim`, names, unary
forms, and general argument syntax need separately bounded evidence.
...
- The checked-in seed gains one lexical M7 target-existence witness. It adds no
  ...
  surface.
- `bright` is a recognition condition, not a general `Truth` parser. `dim`,
  names, `not`, other expressions, and multi-argument forms retain the full
  compiler or a separately accepted pilot.
...
4. `dim`, names, a repeated `bright` argument, missing destination, delayed,
...
`spawn call target bright into destination` line. It does not claim `dim`,
names, `not`, general `Truth` expressions, Whole literals beyond ADR-121/122/123,
