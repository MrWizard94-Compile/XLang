...
| Data-layout choice and generic specialization should be explicit or proven semantics-preserving. | **Implemented, bounded M6 scope** | Odin SoA evidence plus [ADR-009](../ADR-009-m6-explicit-layout-shapes.md): dual-layout Whole tables with equivalence tests and a published harness; automatic rewrite remains out of scope. | No automatic layout rewrite without semantic-equivalence tests and a benchmark protocol. |
...
| Structured concurrency | **Met in M7:** lexical `together`/`spawn` nursery with join and first-failure cancel of remaining unstarted spawns. | No task survives its scope; cancel is deterministic and source-ordered; outer M4 handle discharges nursery errors. | Detached background work or hidden scheduler ownership is necessary. |
...
tests are in [../CORE_CLAIMS.md](../../Current%20state/CORE_CLAIMS.md).
...
approval-gated path is [../ROADMAP.md](../../Current%20state/ROADMAP.md).
