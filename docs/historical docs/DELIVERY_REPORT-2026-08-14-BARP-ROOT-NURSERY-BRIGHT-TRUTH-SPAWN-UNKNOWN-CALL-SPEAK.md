# Delivery Report — BARP root nursery exact bright Truth spawn unknown-target seed-SPEAK pilot

**Date:** 2026-08-14
**Status:** Implementation and release verification complete
**Summary:** ADR-124 extends the bounded `AE-SEED-011` nursery witness with one exact `bright` `Truth` literal without turning the seed into an M7 parser, task checker, or scheduler.
...
two-digit branches decline a source, the checked-in seed recognizes a literal
ordinary total-Whole root `together:` only when its next physical line is
exactly four-space `spawn call target bright into destination`. The opaque
target must be nonempty, the literal span after its first delimiter must be
exactly `bright into `, and a destination suffix must remain.
...
verified, and executable with `bright`.

## Delivery contents

| Artifact | Purpose |
|---|---|
| `seed/aether_seed.ae` and `seed/aether_seed.aeth` | Exact `bright into ` branch after ADR-123 and rebuilt checked-in seed |
| `crates/xlang-core/src/lib.rs` | Explicit ADR-124 tracker and bounded exact-Truth nursery-call contract documentation |
| `crates/xlang-core/tests/seed_self_host.rs` | Red/green direct, packet, product-origin, valid identity/run, lexical/signature/caller-state/delimiter/priority tests |
| ADR-124 and BARP matrix | Decision plus direct, valid, boundary, priority, and product evidence |
...
|---|---|
| Test-driven regression | PASS — the ADR-123 seed emitted no direct packet for a canonical bright root-nursery spawn unknown target |
...
| Full Constitution/release gate | PASS — 2026-08-14: pack v5.0.1, 355 Markdown files / 1,410 local links, zero-warning workspace build and test corpus, four-way seed identity `2F310B043653185DBE62ED61DB4D78A0DD8F2B0D20DD4B978DE634E05CF2ED68`, and a 440-file technical-preview package plus `SHA-256SUMS` with independent consumer verification |
...
- `cargo test -p aether-core --test seed_self_host barp_adr124_seed_speaks_canonical_root_nursery_bright_truth_spawn_unknown_calls`
