...
**Later scoped extensions:** ADR-123 independently adds only one immediate
positive two-digit decimal-Whole argument shape (`10` through `99`), and ADR-124
independently adds only one immediate exact-`bright` Truth argument shape. ADR-125
independently adds only one immediate exact-`dim` Truth argument shape; neither
...
| False-positive boundary | PASS — three-or-more-digit, negative, name/`not dim`/general-Truth, multi-argument, delayed, and descendant children emit no pilot packet |
...
broaden this one-digit delivery.
