...
ADR-128/129/130 add separate, bounded seed-native source-bundle routes. A
caller may supply a v1 `aether.seed-bundle/v1` library -> entry file, a v2
`aether.seed-bundle/v2` foundation -> bridge -> entry file, or a v3
`aether.seed-bundle/v3` left leaf + right leaf -> merge -> entry file to default
`compile` or explicit `forge-bundle`. The host carries opaque Text to the
verified seed `compile_bundle` weave; the seed alone validates the fixed frame,
imports, names, and source profile before producing one compiled source. The
route does not invoke host M11/M22 elaboration and grants no file, resolver,
network, or callback authority to seed code. These are fixed profiles, not a
general seed-native module graph. See
