...
| Core crate | `aether-core` at `crates/xlang-core` (package version 0.28.0) |
| CLI binary | `aether` via `apps/xlang-cli` (package `aether-cli` 0.28.0) |
...
| Product compile | Seed-hosted (`compile_with_seed` / embedded `SEED_COMPILER_ARTIFACT`) for the documented seed surface, including M19a `release`, M19b nursery×resource Policy A, multi-module stdlib layer 1, and project `role: test` |
