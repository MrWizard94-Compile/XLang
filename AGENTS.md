...
parity is not claimed for the seed. M23 product path materializes folded
literals before seed emission (honest seed-surface limit).
...
| Product compile | Seed-hosted for documented surface; M23 pure comptime calls via bootstrap fold + dual-compare; M19e task frames v12 |
...
4. **Honest self-host claims** — dual-compare only where proven (including M19e/M23 corpora as documented); M23 does not claim seed independently parses raw `comptime bind <- call` syntax; do not claim full diagnostic parity.
