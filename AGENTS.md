...
| Core crate | `aether-core` at `crates/xlang-core` (package version 0.26.0) |
| CLI binary | `aether` via `apps/xlang-cli` (package `aether-cli` 0.26.0) |
...
| Product compile | Seed-hosted (`compile_with_seed` / embedded `SEED_COMPILER_ARTIFACT`) for the documented seed surface, including M19a `release` and M19b nursery×resource Policy A |
